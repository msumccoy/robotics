...
class Robot:
    ...
    def send_motion_cmd(self, motion_number):
        ...
        """
        if debug_robot:
            print("sending motion ", motion_number)
        if motion_number == "stop":
            motion_number = 1
        # self.ser.write(variables.stop_motion)
        self.ser.write("\x09\x00\x02\x00\x00\x00\x10\x83\x9e")
        if debug_robot:
            print("sending motion ", motion_number, " after write")
        self.clear_cache()
        motion_cmd = self.generate_motion_cmd(motion_number)
        if debug_robot:
            print("sending motion ", motion_number, " after hex generation")
